...
It should be noted that pg_cast does not represent every type conversion that the system knows how to perform; only those that cannot be deduced from some generic rule.
For example, casting between a domain and its base type is not explicitly represented in pg_cast.
...
:::{.table}
  ...
  :::{.row}
  :::{.cell}
   castsource `oid` (references [pg_type](#catalog-pg-type).oid)
  ...
  :::{.row}
  :::{.cell}
   casttarget `oid` (references [pg_type](#catalog-pg-type).oid)
  ...
  :::{.row}
  :::{.cell}
   castfunc `oid` (references [pg_proc](#catalog-pg-proc).oid)
