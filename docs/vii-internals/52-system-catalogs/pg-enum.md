...
The internal representation of a given enum value is actually the OID of its associated row in pg_enum.
...
:::{.table}
  ...
  :::{.row}
  :::{.cell}
   enumtypid `oid` (references [pg_type](#catalog-pg-type).oid)

   The OID of the [pg_type](#catalog-pg-type) entry owning this enum value
  :::{/cell}
  :::{/row}
  :::{.row}
  :::{.cell}
