...
Prior to PostgreSQL 9.0, there was no permission structure associated with large objects.
As a result, pg_largeobject was publicly readable and could be used to obtain the OIDs (and contents) of all large objects in the system.
This is no longer the case; use [pg_largeobject_metadata](#catalog-pg-largeobject-metadata) to obtain a list of large object OIDs.
...
:::{.table}
  ...
  :::{.row}
  :::{.cell}
   loid `oid` (references [pg_largeobject_metadata](#catalog-pg-largeobject-metadata).oid)
