...
This information allows `DROP` commands to find which other objects must be dropped by `DROP CASCADE` or prevent dropping in the `DROP RESTRICT` case.
...
:::{/item}
:::{.item term="`DEPENDENCY_INTERNAL` (`i`)"}
...
:::{/item}
:::{.item term="`DEPENDENCY_PARTITION_PRI` (`P`); `DEPENDENCY_PARTITION_SEC` (`S`)"}
...
:::{/item}
:::{.item term="`DEPENDENCY_EXTENSION` (`e`)"}
...
:::{/item}
:::{.item term="`DEPENDENCY_AUTO_EXTENSION` (`x`)"}
