...
See [Concurrency Control](#concurrency-control) for more discussion of locking.

pg_locks contains one row per active lockable object, requested lock mode, and relevant process.
Thus, the same lockable object might appear many times, if multiple processes are holding or waiting for locks on it.
However, an object that currently has no locks on it will not appear at all.

There are several distinct types of lockable objects: whole relations (e.g., tables), individual pages of relations, individual tuples of relations, transaction IDs (both virtual and permanent IDs), and general database objects (identified by class OID and object OID, in the same way as in [pg_description](#catalog-pg-description) or [pg_depend](#catalog-pg-depend)).
Also, the right to extend a relation is represented as a separate lockable object, as is the right to update pg_database.datfrozenxid.
...
:::{.table}
  ...
  :::{.cell}
   ...
   Type of the lockable object: `relation`, `extend`, `frozenid`, `page`, `tuple`, `transactionid`, `virtualxid`, `spectoken`, `object`, `userlock`, `advisory`, or `applytransaction`. (See also [Wait Event Types](#wait-event-types).)
  :::{/cell}
  :::{/row}
  :::{.row}
  :::{.cell}
   database `oid` (references [pg_database](#catalog-pg-database).oid)
  ...
  :::{.row}
  :::{.cell}
   relation `oid` (references [pg_class](#catalog-pg-class).oid)
  ...
  :::{.cell}
   ...
   Virtual ID of the transaction targeted by the lock, or null if the target is not a virtual transaction ID; see [Transaction Processing](#transaction-processing)
  :::{/cell}
  :::{/row}
  :::{.row}
  :::{.cell}
   ...
   ID of the transaction targeted by the lock, or null if the target is not a transaction ID; [Transaction Processing](#transaction-processing)
  :::{/cell}
  :::{/row}
  :::{.row}
  :::{.cell}
  ...
  :::{.cell}
   ...
   Name of the lock mode held or desired by this process (see [Table-Level Locks](braised:ref/explicit-locking#table-level-locks) and [Serializable Isolation Level](braised:ref/transaction-iso#serializable-isolation-level))
  :::{/cell}
  :::{/row}
  :::{.row}
  :::{.cell}
