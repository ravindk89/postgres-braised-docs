...
It\'s stored alongside the main relation data in a separate relation fork, named after the filenode number of the relation, plus a `_vm` suffix.
...
This information can also be used by [index-only scans](#indexes-index-only-scans) to answer queries using only the index tuple.
...
The [F.36. pg_visibility — visibility map information and utilities](braised:ref/pgvisibility) module can be used to examine the information stored in the visibility map.
