---
title: "54.4. Streaming Replication Protocol"
...
A Boolean value of `true` (or `on`, `yes`, `1`) tells the backend to go into physical replication walsender mode, wherein a small set of replication commands, shown below, can be issued instead of SQL statements.

Passing `database` as the value for the `replication` parameter instructs the backend to go into logical replication walsender mode, connecting to the database specified in the `dbname` parameter.
In logical replication walsender mode, the replication commands shown below as well as normal SQL commands can be issued.
...
However, it is often more useful to use [pg_receivewal](braised:ref/app-pgreceivewal) (for physical replication) or [pg_recvlogical](braised:ref/app-pgrecvlogical) (for logical replication).

Replication commands are logged in the server log when [log_replication_commands (boolean)
      
       log_replication_commands configuration parameter](braised:ref/runtime-config-logging#log-replication-commands-boolean-log-replication-commands-configuration-parameter) is enabled.

The commands accepted in replication mode are:

:::{.dl}
:::{.item term="`IDENTIFY_SYSTEM`"}
...
:::{/item}
:::{.item term="`SHOW` *name*"}
Requests the server to send the current setting of a run-time parameter. This is similar to the SQL command [SHOW](braised:ref/sql-show).

*name*

:   The name of a run-time parameter. Available parameters are documented in [Server Configuration](#server-configuration).
:::{/item}
:::{.item term="`TIMELINE_HISTORY` *tli*"}
...
:::{/item}
:::{.item term="`CREATE_REPLICATION_SLOT` *slot_name* \[ `TEMPORARY` \] { `PHYSICAL` \| `LOGICAL` *output_plugin* } \[ ( *option* \[, \...\] ) \]"}
Create a physical or logical replication slot. See [Replication Slots](braised:ref/warm-standby#replication-slots) for more about replication slots.

*slot_name*

:   The name of the slot to create. Must be a valid replication slot name (see [Querying and Manipulating Replication Slots](braised:ref/warm-standby#querying-and-manipulating-replication-slots)).

*output_plugin*

:   The name of the output plugin used for logical decoding (see [Logical Decoding Output Plugins](braised:ref/logicaldecoding-output-plugin)).
...
:::{/item}
:::{.item term="`CREATE_REPLICATION_SLOT` *slot_name* \[ `TEMPORARY` \] { `PHYSICAL` \[ `RESERVE_WAL` \] \| `LOGICAL` *output_plugin* \[ `EXPORT_SNAPSHOT` \| `NOEXPORT_SNAPSHOT` \| `USE_SNAPSHOT` \| `TWO_PHASE` \] }"}
For compatibility with older releases, this alternative syntax for the `CREATE_REPLICATION_SLOT` command is still supported.
:::{/item}
:::{.item term="`ALTER_REPLICATION_SLOT` *slot_name* ( *option* \[, \...\] )"}
Change the definition of a replication slot. See [Replication Slots](braised:ref/warm-standby#replication-slots) for more about replication slots. This command is currently only supported for logical replication slots.

*slot_name*

:   The name of the slot to alter. Must be a valid replication slot name (see [Querying and Manipulating Replication Slots](braised:ref/warm-standby#querying-and-manipulating-replication-slots)).
...
:::{/item}
:::{.item term="`READ_REPLICATION_SLOT` *slot_name*"}
...
:::{/item}
:::{.item term="`START_REPLICATION` \[ `SLOT` *slot_name* \] \[ `PHYSICAL` \] *XXX/XXX* \[ `TIMELINE` *tli* \]"}
...
WAL data is sent as a series of CopyData messages; see [Message Data Types](braised:ref/protocol-message-types) and [Message Formats](braised:ref/protocol-message-formats) for details. (This allows other information to be intermixed; in particular the server can send an ErrorResponse message if it encounters a failure after beginning to stream.) The payload of each CopyData message from server to the client contains a message of one of the following formats:
...
:::{/item}
:::{.item term="`START_REPLICATION` `SLOT` *slot_name* `LOGICAL` *XXX/XXX* \[ ( *option_name* \[ *option_value* \] \[, \...\] ) \]"}
Instructs server to start streaming WAL for logical replication, starting at either WAL location *XXX/XXX* or the slot\'s `confirmed_flush_lsn` (see [pg_replication_slots](braised:ref/view-pg-replication-slots)), whichever is greater. This behavior makes it easier for clients to avoid updating their local LSN status when there is no data to process. However, starting at a different LSN than requested might not catch certain kinds of client errors; so the client may wish to check that `confirmed_flush_lsn` matches its expectations before issuing `START_REPLICATION`.
...
:   The name of an option passed to the slot\'s logical decoding output plugin. See [Logical Streaming Replication Protocol](braised:ref/protocol-logical-replication) for options that are accepted by the standard (`pgoutput`) plugin.

*option_value*

:   Optional value, in the form of a string constant, associated with the specified option.
:::{/item}
:::{.item term="`DROP_REPLICATION_SLOT` *slot_name* \[`WAIT`\]"}
...
:   Sets the label of the backup. If none is specified, a backup label of `base backup` will be used. The quoting rules for the label are the same as a standard SQL string with [standard_conforming_strings (boolean)
  stringsstandard conforming
      
   standard_conforming_strings configuration parameter](braised:ref/runtime-config-compatible#standard-conforming-strings-boolean-stringsstandard-conforming-standard-conforming-strings-configuration-parameter) turned on.
