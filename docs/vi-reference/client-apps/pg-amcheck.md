...
layout: reference
id: app-pgamcheck
description: "checks for corruption in one or more
...
pg_amcheck supports running [F.1. amcheck — tools to verify table and index consistency](braised:ref/amcheck)\'s corruption checking functions against one or more databases, with options to select which schemas, tables and indexes to check, which kinds of checking to perform, and whether to perform the checks in parallel, and if so, the number of parallel connections to establish and use.

Only ordinary and toast table relations, materialized views, sequences, and btree indexes are currently supported.
Other relation types are silently skipped.

If `dbname` is specified, it should be the name of a single database to check, and no other database selection options should be present.
Otherwise, if any database selection options are present, all matching databases will be checked.
If no such options are present, the default database will be checked.
Database selection options include `--all`, `--database` and `--exclude-database`.
They also include `--relation`, `--exclude-relation`, `--table`, `--exclude-table`, `--index`, and `--exclude-index`, but only when such options are used with a three-part pattern (e.g. `mydb*.myschema*.myrel*`).
Finally, they include `--schema` and `--exclude-schema` when such options are used with a two-part pattern (e.g. `mydb*.myschema*`).

*dbname* can also be a [connection string](#libpq-connstring).
...
:::{/item}
:::{.item term="`-d pattern`; `--database=pattern`"}
Check databases matching the specified [*pattern*](#app-psql-patterns), except for any excluded by `--exclude-database`. This option can be specified more than once.
:::{/item}
:::{.item term="`-D pattern`; `--exclude-database=pattern`"}
Exclude databases matching the given [*pattern*](#app-psql-patterns). This option can be specified more than once.
:::{/item}
:::{.item term="`-i pattern`; `--index=pattern`"}
Check indexes matching the specified [*pattern*](#app-psql-patterns), unless they are otherwise excluded. This option can be specified more than once.

This is similar to the `--relation` option, except that it applies only to indexes, not to other relation types.
:::{/item}
:::{.item term="`-I pattern`; `--exclude-index=pattern`"}
Exclude indexes matching the specified [*pattern*](#app-psql-patterns). This option can be specified more than once.

This is similar to the `--exclude-relation` option, except that it applies only to indexes, not other relation types.
:::{/item}
:::{.item term="`-r pattern`; `--relation=pattern`"}
Check relations matching the specified [*pattern*](#app-psql-patterns), unless they are otherwise excluded. This option can be specified more than once.
...
:::{/item}
:::{.item term="`-R pattern`; `--exclude-relation=pattern`"}
Exclude relations matching the specified [*pattern*](#app-psql-patterns). This option can be specified more than once.

As with `--relation`, the [*pattern*](#app-psql-patterns) may be unqualified, schema-qualified, or database- and schema-qualified.
:::{/item}
:::{.item term="`-s pattern`; `--schema=pattern`"}
Check tables and indexes in schemas matching the specified [*pattern*](#app-psql-patterns), unless they are otherwise excluded. This option can be specified more than once.
...
:::{/item}
:::{.item term="`-S pattern`; `--exclude-schema=pattern`"}
Exclude tables and indexes in schemas matching the specified [*pattern*](#app-psql-patterns). This option can be specified more than once.

As with `--schema`, the pattern may be database-qualified.
:::{/item}
:::{.item term="`-t pattern`; `--table=pattern`"}
Check tables matching the specified [*pattern*](#app-psql-patterns), unless they are otherwise excluded. This option can be specified more than once.
...
Exclude tables matching the specified [*pattern*](#app-psql-patterns). This option can be specified more than once.
...
:::{/item}
:::{.item term="`--no-dependent-indexes`"}
...
:::{.dl}
:::{.item term="`--exclude-toast-pointers`"}
...
:::{.dl}
:::{.item term="`--checkunique`"}
For each index with unique constraint checked, verify that no more than one among duplicate entries is visible in the index using [F.1. amcheck — tools to verify table and index consistency](braised:ref/amcheck)\'s `checkunique` option.
:::{/item}
:::{.item term="`--heapallindexed`"}
For each index checked, verify the presence of all heap tuples as index tuples in the index using [F.1. amcheck — tools to verify table and index consistency](braised:ref/amcheck)\'s `heapallindexed` option.
:::{/item}
:::{.item term="`--parent-check`"}
For each btree index checked, use [F.1. amcheck — tools to verify table and index consistency](braised:ref/amcheck)\'s `bt_index_parent_check` function, which performs additional checks of parent/child relationships during index checking.
...
:::{/item}
:::{.item term="`--rootdescend`"}
For each index checked, re-find tuples on the leaf level by performing a new search from the root page for each tuple using [F.1. amcheck — tools to verify table and index consistency](braised:ref/amcheck)\'s `rootdescend` option.
...
Specifies a database or [connection string](#libpq-connstring) to be used to discover the list of databases to be checked. If neither `--all` nor any option including a database pattern is used, no such connection is required and this option does nothing. Otherwise, any connection string parameters other than the database name which are included in the value for this option will also be used when connecting to the databases being checked. If this option is omitted, the default is `postgres` or, if that fails, `template1`.
:::{/item}
:::{/dl}
...
:::{/item}
:::{.item term="`--install-missing`; `--install-missing=schema`"}
...
At present, the only required extension is [F.1. amcheck — tools to verify table and index consistency](braised:ref/amcheck).
:::{/item}
:::{.item term="`-?`; `--help`"}
...
`pg_amcheck`, like most other PostgreSQL utilities, also uses the environment variables supported by libpq (see [Environment Variables](braised:ref/libpq-envars)).
