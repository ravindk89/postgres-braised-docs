...
dropdb is a wrapper around the SQL command [`DROP DATABASE`](#sql-dropdatabase).
...
:::{/item}
:::{.item term="`-f`; `--force`"}
Attempt to terminate all existing connections to the target database before dropping it. See [DROP DATABASE](braised:ref/sql-dropdatabase) for more information on this option.
:::{/item}
...
In case of difficulty, see [DROP DATABASE](braised:ref/sql-dropdatabase) and [psql](braised:ref/app-psql) for discussions of potential problems and error messages.
