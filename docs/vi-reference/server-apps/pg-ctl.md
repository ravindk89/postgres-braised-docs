...
pg_ctl is a utility for initializing a PostgreSQL database cluster, starting, stopping, or restarting the PostgreSQL database server ([postgres](braised:ref/app-postgres)), or displaying the status of a running server.
Although the server can be started manually, pg_ctl encapsulates tasks such as redirecting log output and properly detaching from the terminal and process group.
It also provides convenient options for controlled shutdown.

The `init` or `initdb` mode creates a new PostgreSQL database cluster, that is, a collection of databases that will be managed by a single server instance.
This mode invokes the `initdb` command.
See [initdb](braised:ref/app-initdb) for details.

`start` mode launches a new server.
The server is started in the background, and its standard input is attached to `/dev/null` (or `nul` on Windows).
...
Three different shutdown methods can be selected with the `-m` option. "Smart" mode disallows new connections, then waits for all existing clients to disconnect.
If the server is in hot standby, recovery and streaming replication will be terminated once all clients have disconnected. "Fast" mode (the default) does not wait for clients to disconnect.
All active transactions are rolled back and clients are forcibly disconnected, then the server is shut down. "Immediate" mode will abort all server processes immediately, without a clean shutdown.
...
This allows changing the `postgres` command-line options, or changing configuration-file options that cannot be changed without restarting the server.
...
`reload` mode simply sends the `postgres` server process a `SIGHUP` signal, causing it to reread its configuration files (`postgresql.conf`, `pg_hba.conf`, etc.).
This allows changing configuration-file options that do not require a full server restart to take effect.

`status` mode checks whether a server is running in the specified data directory.
If it is, the server\'s PID and the command line options that were used to invoke it are displayed.
...
For details on how to use this mode with external log rotation tools, see [Log File Maintenance](braised:ref/logfile-maintenance).

`kill` mode sends a signal to a specified process.
This is primarily valuable on Microsoft Windows which does not have a built-in kill command.
...
The `-S` option allows selection of service start type, either "auto" (start service automatically on system startup) or "demand" (start service on demand).

`unregister` mode unregisters a system service on Microsoft Windows.
This undoes the effects of the `register` command.
...
:::{/item}
:::{.item term="`-D datadir`; `--pgdata=datadir`"}
...
:::{/item}
:::{.item term="`-l filename`; `--log=filename`"}
...
:::{/item}
:::{.item term="`-o options`; `--options=options`"}
...
:::{/item}
:::{.item term="`-o initdb-options`; `--options=initdb-options`"}
...
:::{.dl}
:::{.item term="`-e source`"}
Name of the event source for pg_ctl to use for logging to the event log when running as a Windows service. The default is `PostgreSQL`. Note that this only controls messages sent from pg_ctl itself; once started, the server will use the event source specified by its [event_source (string)
      
   event_source configuration parameter](braised:ref/runtime-config-logging#event-source-string-event-source-configuration-parameter) parameter. Should the server fail very early in startup, before that parameter has been set, it might also log using the default event source name `PostgreSQL`.
:::{/item}
:::{.item term="`-N servicename`"}
...
:::{/item}
:::{.item term="`-P password`"}
Password for the user to run the service as.
:::{/item}
:::{.item term="`-S start-type`"}
...
For additional variables that affect the server, see [postgres](braised:ref/app-postgres).

## Files

:::{.dl}
:::{.item term="`postmaster.pid`"}
pg_ctl examines this file in the data directory to determine whether the server is currently running.
:::{/item}
:::{.item term="`postmaster.opts`"}
...
Restarting the server is almost equivalent to stopping the server and starting it again, except that by default, `pg_ctl` saves and reuses the command line options that were passed to the previously-running instance.
To restart the server using the same options as before, use:
