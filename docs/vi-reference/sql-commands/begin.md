...
`BEGIN` initiates a transaction block, that is, all statements after a `BEGIN` command will be executed in a single transaction until an explicit [`COMMIT`](#sql-commit) or [`ROLLBACK`](#sql-rollback) is given.
...
Execution of multiple statements inside a transaction is also useful to ensure consistency when making several related changes: other sessions will be unable to see the intermediate states wherein not all the related updates have been done.
...
Refer to [SET TRANSACTION](braised:ref/sql-set-transaction) for information on the meaning of the other parameters to this statement.
...
To nest transactions within a transaction block, use savepoints (see [SAVEPOINT](braised:ref/sql-savepoint)).
...
It is equivalent to the SQL-standard command [`START TRANSACTION`](#sql-start-transaction), whose reference page contains additional compatibility information.
...
You are advised to be careful about the transaction semantics when porting database applications.
