...
`CREATE TABLE AS` bears some resemblance to creating a view, but it is really quite different: it creates a new table and evaluates the query just once to fill the new table initially.
...
:::{.dl}
:::{.item term="`GLOBAL` or `LOCAL`"}
Ignored for compatibility. Use of these keywords is deprecated; refer to [CREATE TABLE](braised:ref/sql-createtable) for details.
:::{/item}
:::{/dl}
...
This clause specifies optional storage parameters for the new table; see [Storage Parameters](braised:ref/sql-createtable#storage-parameters) in the [CREATE TABLE](braised:ref/sql-createtable) documentation for more information. For backward-compatibility the `WITH` clause for a table can also include `OIDS=FALSE` to specify that rows of the new table should contain no OIDs (object identifiers), `OIDS=TRUE` is not supported anymore.
:::{/item}
...
:::{/item}
:::{.item term="*query*"}
A [`SELECT`](#sql-select), `TABLE`, or [`VALUES`](#sql-values) command, or an [`EXECUTE`](#sql-execute) command that runs a prepared `SELECT`, `TABLE`, or `VALUES` query.
:::{/item}
...
This command is functionally similar to [SELECT INTO](braised:ref/sql-selectinto), but it is preferred since it is less likely to be confused with other uses of the `SELECT INTO` syntax.
...
`CREATE TABLE AS` conforms to the SQL standard.
The following are nonstandard extensions:
...
-   PostgreSQL handles temporary tables in a way rather different from the standard; see [CREATE TABLE](braised:ref/sql-createtable) for details.
