...
To change a view\'s schema, you must also have `CREATE` privilege on the new schema.
To alter the owner, you must be able to `SET ROLE` to the new owning role, and that role must have `CREATE` privilege on the view\'s schema. (These restrictions enforce that altering the owner doesn\'t do anything you couldn\'t do by dropping and recreating the view.
...
:::{/item}
:::{.item term="`SET ( view_option_name [= view_option_value] [, ... ] )`; `RESET ( view_option_name [, ... ] )`"}
