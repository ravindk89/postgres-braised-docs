...
For example, when writing a function in PL/Python using the `hstore` type, PL/Python has no prior knowledge how to present `hstore` values in the Python environment.
Language implementations usually default to using the text representation, but that is inconvenient when, for example, an associative array or a list would be more appropriate.
...
It is not necessary to provide both of these functions.
If one is not specified, the language-specific default behavior will be used if necessary. (To prevent a transformation in a certain direction from happening at all, you could also write a transform function that always errors out.)
...
:::{/item}
:::{.item term="`from_sql_function_name[(argument_type [, ...])]`"}
...
:::{/item}
:::{.item term="`to_sql_function_name[(argument_type [, ...])]`"}
...
This form of `CREATE TRANSFORM` is a PostgreSQL extension.
There is a `CREATE TRANSFORM` command in the SQL standard, but it is for adapting data types to client languages.
That usage is not supported by PostgreSQL.

## See Also

## See Also

[CREATE FUNCTION](braised:ref/sql-createfunction), [CREATE LANGUAGE](braised:ref/sql-createlanguage), [CREATE TYPE](braised:ref/sql-createtype), [DROP TRANSFORM](braised:ref/sql-droptransform)
