...
:::{.dl}
:::{.item term="`spi_exec_query(query [, limit])`"}
...
:::{/item}
:::{.item term="`spi_query(command)`; `spi_fetchrow(cursor)`; `spi_cursor_close(cursor)`"}
...
:::{/item}
:::{.item term="`spi_prepare(command, argument types)`; `spi_query_prepared(plan, arguments)`; `spi_exec_prepared(plan [, attributes], arguments)`; `spi_freeplan(plan)`"}
...
Another example illustrates usage of an optional parameter in `spi_exec_prepared`:
    ...
    (2 rows)
:::{/item}
:::{.item term="`spi_commit()`; `spi_rollback()`"}
...
:::{/item}
:::{.item term="`quote_literal(string)`"}
...
:::{/item}
:::{.item term="`quote_nullable(string)`"}
...
:::{/item}
:::{.item term="`decode_bytea(string)`"}
Return the unescaped binary data represented by the contents of the given string, which should be `bytea` encoded.
:::{/item}
:::{.item term="`encode_bytea(string)`"}
Return the `bytea` encoded form of the binary data contents of the given string.
:::{/item}
:::{.item term="`encode_array_literal(array)`; `encode_array_literal(array, delimiter)`"}
Returns the contents of the referenced array as a string in array literal format (see [Array Value Input](braised:ref/arrays#array-value-input)). Returns the argument value unaltered if it\'s not a reference to an array. The delimiter used between elements of the array literal defaults to \"`,`\" if a delimiter is not specified or is undef.
:::{/item}
:::{.item term="`encode_typed_literal(value, typename)`"}
...
:::{/item}
:::{.item term="`encode_array_constructor(array)`"}
Returns the contents of the referenced array as a string in array constructor format (see [Array Constructors](braised:ref/sql-expressions#array-constructors)). Individual values are quoted using `quote_nullable`. Returns the argument value, quoted using `quote_nullable`, if it\'s not a reference to an array.
:::{/item}
:::{.item term="`looks_like_number(string)`"}
...
:::{/item}
:::{.item term="`is_array_ref(argument)`"}
