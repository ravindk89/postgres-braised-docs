---
title: "43.5. Trusted and Untrusted PL/Perl"
id: plperl-trusted
---

## Trusted and Untrusted PL/Perl

Normally, PL/Perl is installed as a "trusted" programming language named `plperl`.
...
There is no way to access internals of the database server process or to gain OS-level access with the permissions of the server process, as a C function can do.
...
For example, one might want a Perl function that sends mail.
To handle these cases, PL/Perl can also be installed as an "untrusted" language (usually called PL/PerlU`plperlu` will select the untrusted PL/Perl variant.
