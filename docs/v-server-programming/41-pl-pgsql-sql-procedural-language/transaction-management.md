---
title: "41.8. Transaction Management"
...
In cases where transactions are committed in a loop, it might be desirable to start new transactions automatically with the same characteristics as the previous one.
The commands `COMMIT AND CHAIN` and `ROLLBACK AND CHAIN` accomplish this.
...
For example, if the call stack is `CALL proc1()` `CALL proc2()` `CALL proc3()`, then the second and third procedures can perform transaction control actions.
...
PL/pgSQL does not support savepoints (`SAVEPOINT`/`ROLLBACK TO SAVEPOINT`/`RELEASE SAVEPOINT` commands).
Typical usage patterns for savepoints can be replaced by blocks with exception handlers (see [Trapping Errors](braised:ref/plpgsql-control-structures#trapping-errors)).
...
The cursor is still removed automatically after the loop, so this is mostly invisible to the user.
But one must keep in mind that any table or row locks taken by the cursor\'s query will no longer be held after the first `COMMIT` or `ROLLBACK`.

Transaction commands are not allowed in cursor loops driven by commands that are not read-only (for example `UPDATE ...
RETURNING`).
