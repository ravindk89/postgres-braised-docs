...
For information about new features in major release 18, see [E.4. Release 18](braised:ref/release-18).

### Migration to Version 18.2

A dump/restore is not required for those running 18.X.

However, if you have any indexes on `ltree` columns, it may be necessary to reindex them after updating.
See the sixth changelog entry below.
...
-   Add new server parameter [file_extend_method (enum)
      
       file_extend_method configuration parameter](braised:ref/runtime-config-resource#file-extend-method-enum-file-extend-method-configuration-parameter) to control use of `posix_fallocate()` (Thomas Munro) [](https://postgr.es/c/33e3de6d7)
