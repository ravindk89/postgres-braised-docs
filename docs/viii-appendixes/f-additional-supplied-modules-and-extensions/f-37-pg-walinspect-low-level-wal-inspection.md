---
title: "F.37. pg_walinspect — low-level WAL inspection"
id: pgwalinspect
---

## pg_walinspect low-level WAL inspection

The `pg_walinspect` module provides SQL functions that allow you to inspect the contents of write-ahead log of a running PostgreSQL database cluster at a low level, which is useful for debugging, analytical, reporting or educational purposes.
It is similar to [pg_waldump](braised:ref/pgwaldump), but accessible through SQL rather than a separate utility.
...
:::{.dl}
:::{.item term="`pg_get_wal_record_info(in_lsn pg_lsn) returns record`"}
...
:::{/item}
:::{.item term="`pg_get_wal_records_info(start_lsn pg_lsn, end_lsn pg_lsn) returns setof record`"}
...
:::{/item}
:::{.item term="`pg_get_wal_block_info(start_lsn pg_lsn, end_lsn pg_lsn, show_data boolean DEFAULT true) returns setof record`"}
...
:::{.callout type="tip"}
The `pg_filenode_relation` function (see [Database Object Location Functions](braised:ref/functions-admin#database-object-location-functions)) can help you to determine which relation was modified during original execution.
:::
...
The function raises an error if *start_lsn* is not available.
:::{/item}
:::{.item term="`pg_get_wal_stats(start_lsn pg_lsn, end_lsn pg_lsn, per_record boolean DEFAULT false) returns setof record`"}
