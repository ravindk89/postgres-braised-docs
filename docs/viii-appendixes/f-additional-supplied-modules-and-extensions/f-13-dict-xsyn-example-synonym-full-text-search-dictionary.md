---
title: "F.13. dict_xsyn — example synonym full-text search dictionary"
id: dict-xsyn
---

## dict_xsyn example synonym full-text search dictionary

`dict_xsyn` (Extended Synonym Dictionary) is an example of an add-on dictionary template for full-text search.
This dictionary type replaces words with groups of their synonyms, and so makes it possible to search for a word using any of its synonyms.
...
Installing the `dict_xsyn` extension creates a text search template `xsyn_template` and a dictionary `xsyn` based on it, with default parameters.
...
Real-world usage will involve including it in a text search configuration as described in [Full Text Search](#full-text-search).
