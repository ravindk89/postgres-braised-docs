---
title: "F.34. pg_surgery — perform low-level surgery on relation data"
id: pgsurgery
---
...
For example, these functions can easily be used to make a table inconsistent with its own indexes, to cause `UNIQUE` or `FOREIGN KEY` constraint violations, or even to make tuples visible which, when read, will cause a database server crash.
They should be used with great caution and only as a last resort.

### Functions

:::{.dl}
:::{.item term="`heap_force_kill(regclass, tid[]) returns void`"}
`heap_force_kill` marks "used" line pointers as "dead" without examining the tuples. The intended use of this function is to forcibly remove tuples that are not otherwise accessible. For example:
    ...
    (0 rows)
:::{/item}
:::{.item term="`heap_force_freeze(regclass, tid[]) returns void`"}
