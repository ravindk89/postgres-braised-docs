---
title: "F.17. hstore — hstore key/value datatype"
id: hstore
---
...
Some examples: k =\> v foo =\> bar, baz =\> whatever \"1-a\" =\> \"anything at all\" The order of the pairs is not significant (and may not be reproduced on output).
...
If you declare an `hstore` with duplicate keys, only one will be stored in the `hstore` and there is no guarantee as to which will be kept:
...
The operators provided by the `hstore` module are shown in Operators, the functions in Functions.
