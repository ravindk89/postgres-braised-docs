---
title: "F.3. auto_explain — log execution plans of slow queries"
id: auto-explain
---

## auto_explain log execution plans of slow queries

The `auto_explain` module provides a means for logging execution plans of slow statements automatically, without having to run [EXPLAIN](braised:ref/sql-explain) by hand.
This is especially helpful for tracking down un-optimized queries in large applications.

The module provides no SQL-accessible functions.
To use it, simply load it into the server.
You can load it into an individual session:

    LOAD 'auto_explain';

(You must be superuser to do that.) More typical usage is to preload it into some or all sessions by including `auto_explain` in [session_preload_libraries (string)
...
:::{.dl}
:::{.item term="`auto_explain.log_min_duration` (`integer`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_parameter_max_length` (`integer`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_analyze` (`boolean`)"}
...
:::
:::{/item}
:::{.item term="`auto_explain.log_buffers` (`boolean`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_wal` (`boolean`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_timing` (`boolean`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_triggers` (`boolean`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_verbose` (`boolean`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_settings` (`boolean`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_format` (`enum`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_level` (`enum`)"}
...
:::{/item}
:::{.item term="`auto_explain.log_nested_statements` (`boolean`)"}
...
:::{/item}
:::{.item term="`auto_explain.sample_rate` (`real`)"}
...
In ordinary usage, these parameters are set in `postgresql.conf`, although superusers can alter them on-the-fly within their own sessions.
