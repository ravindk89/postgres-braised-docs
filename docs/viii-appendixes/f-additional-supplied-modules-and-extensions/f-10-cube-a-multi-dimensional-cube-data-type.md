---
title: "F.10. cube — a multi-dimensional cube data type"
id: cube
---
...
Cube External Representations shows the valid external representations for the `cube` type. *x*, *y*, etc. denote floating-point numbers.
...
Cube Operators shows the specialized operators provided for type `cube`.
...
In addition to the above operators, the usual comparison operators shown in [Comparison Operators](braised:ref/functions-comparison#comparison-operators) are available for type `cube`. These operators first compare the first coordinates, and if those are equal, compare the second coordinates, etc. They exist mainly to support the b-tree index operator class for `cube`, which can be useful for example if you would like a UNIQUE constraint on a `cube` column. Otherwise, this ordering is not of much practical use.
...
Cube Functions shows the available functions.
