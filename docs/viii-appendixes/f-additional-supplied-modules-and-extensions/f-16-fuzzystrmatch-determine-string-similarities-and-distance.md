---
title: "F.16. fuzzystrmatch — determine string similarities and distance"
id: fuzzystrmatch
---
...
It was initially used by the United States Census in 1880, 1900, and 1910.
...
Since Soundex codes have four characters, the result ranges from zero to four, with zero being no match and four being an exact match. (Thus, the function is misnamed `similarity` would have been a better name.)
...
Like the original Soundex system, Daitch-Mokotoff Soundex matches similar-sounding names by converting them to the same code.
However, Daitch-Mokotoff Soundex is significantly more useful for non-English names than the original system.
...
For matching of single names, returned text arrays can be matched directly using the `&&` operator: any overlap can be considered a match.
A GIN index may be used for efficiency, see [GIN Indexes](braised:ref/gin) and this example:
...
For indexing and matching of any number of names in any order, Full Text Search features can be used.
See [Full Text Search](#full-text-search) and this example:
...
If it is desired to avoid recalculation of soundex codes during index rechecks, an index on a separate column can be used instead of an index on an expression.
A stored generated column can be used for this; see [Generated Columns](braised:ref/ddl-generated-columns).
...
If `max_d` is negative then the behavior is the same as `levenshtein`.
...
The Double Metaphone system computes two "sounds like" strings for a given input string a "primary" and an "alternate".
In most cases they are the same, but for non-English names especially they can be a bit different, depending on pronunciation.
