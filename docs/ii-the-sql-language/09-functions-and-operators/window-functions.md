...
See [Window Functions](braised:ref/tutorial-window) for an introduction to this feature, and [Window Function Calls](braised:ref/sql-expressions#window-function-calls) for syntax details.

The built-in window functions are listed in General-Purpose Window Functions.
Note that these functions *must* be invoked using window function syntax, i.e., an `OVER` clause is required.

In addition to these functions, any built-in or user-defined ordinary aggregate (i.e., not ordered-set or hypothetical-set aggregates) can be used as a window function; see [Aggregate Functions](braised:ref/functions-aggregate) for a list of the built-in aggregates.
Aggregate functions act as window functions only when an `OVER` clause follows the call; otherwise they act as plain aggregates and return a single row for the entire set.
...
Note that `first_value`, `last_value`, and `nth_value` consider only the rows within the "window frame", which by default contains the rows from the start of the partition through the last peer of the current row. This is likely to give unhelpful results for `last_value` and sometimes also `nth_value`. You can redefine the frame by adding a suitable frame specification (`RANGE`, `ROWS` or `GROUPS`) to the `OVER` clause. See [Window Function Calls](braised:ref/sql-expressions#window-function-calls) for more information about frame specifications.
