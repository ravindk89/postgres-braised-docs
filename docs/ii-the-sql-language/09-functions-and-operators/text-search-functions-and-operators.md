---
title: "9.13. Text Search Functions and Operators"
id: functions-textsearch
---

## Text Search Functions and Operators

Text Search Operators, Text Search Functions and Text Search Debugging Functions summarize the functions and operators that are provided for full text searching.
See [Full Text Search](#full-text-search) for a detailed explanation of PostgreSQL\'s text search facility.
...
In addition to these specialized operators, the usual comparison operators shown in [Comparison Operators](braised:ref/functions-comparison#comparison-operators) are available for types `tsvector` and `tsquery`. These are not very useful for text searching but allow, for example, unique indexes to be built on columns of these types.
...
:::{.table}
  ...
  :::{.cell}
   ...
   Returns the OID of the current default text search configuration (as set by [default_text_search_config (string)
      
       default_text_search_config configuration parameter](braised:ref/runtime-config-client#default-text-search-config-string-default-text-search-config-configuration-parameter)).
  ...
  :::{.cell}
   ...
   Displays, in an abbreviated form, the match(es) for the `query` in the `document`, which must be raw text not a `tsvector`. Words in the document are normalized according to the specified or default configuration before matching to the query. Use of this function is discussed in [Highlighting Results](braised:ref/textsearch-controls#highlighting-results), which also describes the available `options`.
  ...
  :::{.cell}
   ...
   Displays, in an abbreviated form, match(es) for the `query` that occur in string values within the JSON `document`. See [Highlighting Results](braised:ref/textsearch-controls#highlighting-results) for more details.
  ...
  :::{.cell}
   ...
   Computes a score showing how well the `vector` matches the `query`. See [Ranking Search Results](braised:ref/textsearch-controls#ranking-search-results) for details.
  ...
  :::{.cell}
   ...
   Computes a score showing how well the `vector` matches the `query`, using a cover density algorithm. See [Ranking Search Results](braised:ref/textsearch-controls#ranking-search-results) for details.
  ...
  :::{.cell}
   ...
   Replaces occurrences of `target` with `substitute` within the `query`. See [Query Rewriting](braised:ref/textsearch-features#query-rewriting) for details.
  ...
  :::{.cell}
   ...
   Replaces portions of the `query` according to target(s) and substitute(s) obtained by executing a `SELECT` command. See [Query Rewriting](braised:ref/textsearch-features#query-rewriting) for details.
...
:::{.callout type="note"}
All the text search functions that accept an optional `regconfig` argument will use the configuration specified by [default_text_search_config (string)
      
       default_text_search_config configuration parameter](braised:ref/runtime-config-client#default-text-search-config-string-default-text-search-config-configuration-parameter) when that argument is omitted.
:::

The functions in Text Search Debugging Functions are listed separately because they are not usually used in everyday text searching operations.
They are primarily helpful for development and debugging of new text search configurations.
...
:::{.table}
  ...
  :::{.cell}
   ...
   Extracts and normalizes tokens from the `document` according to the specified or default text search configuration, and returns information about how each token was processed. See [Configuration Testing](braised:ref/textsearch-debugging#configuration-testing) for details.
  ...
  :::{.cell}
   ...
   Returns an array of replacement lexemes if the input token is known to the dictionary, or an empty array if the token is known to the dictionary but it is a stop word, or NULL if it is not a known word. See [Dictionary Testing](braised:ref/textsearch-debugging#dictionary-testing) for details.
  ...
  :::{.cell}
   ...
   Extracts tokens from the `document` using the named parser. See [Parser Testing](braised:ref/textsearch-debugging#parser-testing) for details.
  ...
  :::{.cell}
   ...
   Extracts tokens from the `document` using a parser specified by OID. See [Parser Testing](braised:ref/textsearch-debugging#parser-testing) for details.
  ...
  :::{.cell}
   ...
   Returns a table that describes each type of token the named parser can recognize. See [Parser Testing](braised:ref/textsearch-debugging#parser-testing) for details.
  ...
  :::{.cell}
   ...
   Returns a table that describes each type of token a parser specified by OID can recognize. See [Parser Testing](braised:ref/textsearch-debugging#parser-testing) for details.
  ...
  :::{.cell}
   ...
   Executes the `sqlquery`, which must return a single `tsvector` column, and returns statistics about each distinct lexeme contained in the data. See [Gathering Document Statistics](braised:ref/textsearch-features#gathering-document-statistics) for details.
