...
PostgreSQL provides several index types: B-tree, Hash, GiST, SP-GiST, GIN, BRIN, and the extension [bloom](#bloom).
Each index type uses a different algorithm that is best suited to different types of indexable clauses.
By default, the [`CREATE INDEX`](#sql-createindex) command creates B-tree indexes, which fit the most common situations.
...
However, if your database does not use the C locale you will need to create the index with a special operator class to support indexing of pattern-matching queries; see [Operator Classes and Operator Families](braised:ref/indexes-opclass) below.
...
As an example, the standard distribution of PostgreSQL includes GiST operator classes for several two-dimensional geometric data types, which support indexed queries using these operators: \<\< &* &* \>\> \<\<\| &\<\| \|&\> \|\>\> @\> \<@ \~= && (See [Geometric Functions and Operators](braised:ref/functions-geometry) for the meaning of these operators.) The GiST operator classes included in the standard distribution are documented in [Built-in GiST Operator Classes](braised:ref/gist#built-in-gist-operator-classes).
Many other GiST operator classes are available in the `contrib` collection or as separate projects.
For more information see [GiST Indexes](braised:ref/gist).
...
In [Built-in GiST Operator Classes](braised:ref/gist#built-in-gist-operator-classes), operators that can be used in this way are listed in the column "Ordering Operators".
...
As an example, the standard distribution of PostgreSQL includes SP-GiST operator classes for two-dimensional points, which support indexed queries using these operators: \<* *\> \~= \<@ \<\<\| \|\>\> (See [Geometric Functions and Operators](braised:ref/functions-geometry) for the meaning of these operators.) The SP-GiST operator classes included in the standard distribution are documented in [Built-in SP-GiST Operator Classes](braised:ref/spgist#built-in-sp-gist-operator-classes).
For more information see [SP-GiST Indexes](braised:ref/spgist).

Like GiST, SP-GiST supports "nearest-neighbor" searches.
For SP-GiST operator classes that support distance ordering, the corresponding operator is listed in the "Ordering Operators" column in [Built-in SP-GiST Operator Classes](braised:ref/spgist#built-in-sp-gist-operator-classes).
...
As an example, the standard distribution of PostgreSQL includes a GIN operator class for arrays, which supports indexed queries using these operators: *@ @* = && (See [Array Functions and Operators](braised:ref/functions-array) for the meaning of these operators.) The GIN operator classes included in the standard distribution are documented in [Built-in GIN Operator Classes](braised:ref/gin#built-in-gin-operator-classes).
Many other GIN operator classes are available in the `contrib` collection or as separate projects.
For more information see [GIN Indexes](braised:ref/gin).
...
This supports indexed queries using these operators: \< *= = *= \> The BRIN operator classes included in the standard distribution are documented in [Built-in BRIN Operator Classes](braised:ref/brin#built-in-brin-operator-classes).
For more information see [BRIN Indexes](braised:ref/brin).
