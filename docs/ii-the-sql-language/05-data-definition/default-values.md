...
A data manipulation command can also request explicitly that a column be set to its default value, without having to know what that value is. (Details about data manipulation commands are in [Data Manipulation](#data-manipulation).)
...
where the `nextval()` function supplies successive values from a sequence object (see [Sequence Manipulation Functions](braised:ref/functions-sequence)).
...
The `SERIAL` shorthand is discussed further in [Serial Types](braised:ref/datatype-numeric#serial-types).
