...
A generated column is by default of the virtual kind.
Use the keywords `VIRTUAL` or `STORED` to make the choice explicit.
See [CREATE TABLE](braised:ref/sql-createtable) for more details.
...
Consider the differences between a column with a default and a generated column.
...
-   Foreign tables can have generated columns. See [CREATE FOREIGN TABLE](braised:ref/sql-createforeigntable) for details.
...
-   Generated columns maintain access privileges separately from their underlying base columns. So, it is possible to arrange it so that a particular role can read from a generated column but not from the underlying base columns.
    ...
    For virtual generated columns, this is only fully secure if the generation expression uses only leakproof functions (see [CREATE FUNCTION](braised:ref/sql-createfunction)), but this is not enforced by the system.
...
-   Generated columns are allowed to be replicated during logical replication according to the `CREATE PUBLICATION` parameter [`publish_generated_columns`](#sql-createpublication-params-with-publish-generated-columns) or by including them in the column list of the `CREATE PUBLICATION` command. This is currently only supported for stored generated columns. See [Generated Column Replication](braised:ref/logical-replication-gencols) for details.
