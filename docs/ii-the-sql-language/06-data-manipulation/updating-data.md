...
To update existing rows, use the [UPDATE](braised:ref/sql-update) command.
...
Recall from [Data Definition](#data-definition) that SQL does not, in general, provide a unique identifier for rows.
...
Many other operators are available (see [Functions and Operators](#functions-and-operators)).
