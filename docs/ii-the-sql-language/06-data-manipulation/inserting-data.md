...
To create a new row, use the [INSERT](braised:ref/sql-insert) command.
The command requires the table name and column values.
For example, consider the products table from [Data Definition](#data-definition):
...
This provides the full power of the SQL query mechanism ([Queries](#queries)) for computing the rows to be inserted.

:::{.callout type="tip"}
When inserting a lot of data at the same time, consider using the [COPY](braised:ref/sql-copy) command. It is not as flexible as the [INSERT](braised:ref/sql-insert) command, but is more efficient. Refer to [Populating a Database](braised:ref/populate) for more information on improving bulk loading performance.
:::
