---
title: "15.2. When Can Parallel Query Be Used?"
id: when-can-parallel-query-be-used
---

There are several settings that can cause the query planner not to generate a parallel query plan under any circumstances.
...
-   [max_parallel_workers_per_gather (integer)
        ...
        max_parallel_workers_per_gather configuration parameter](braised:ref/runtime-config-resource#max-parallel-workers-per-gather-integer-max-parallel-workers-per-gather-configuration-parameter) must be set to a value that is greater than zero. This is a special case of the more general principle that no more workers should be used than the number configured via `max_parallel_workers_per_gather`.

In addition, the system must not be running in single-user mode.
Since the entire database system is running as a single process in this situation, no background workers will be available.
...
-   The query uses any function marked `PARALLEL UNSAFE`. Most system-defined functions are `PARALLEL SAFE`, but user-defined functions are marked `PARALLEL UNSAFE` by default. See the discussion of [Parallel Safety](braised:ref/parallel-safety).
...
If this occurs, the leader will execute the portion of the plan below the `Gather` node entirely by itself, almost as if the `Gather` node were not present.
This will happen if any of the following conditions are met:

-   No background workers can be obtained because of the limitation that the total number of background workers cannot exceed [max_worker_processes (integer)
...
-   No background workers can be obtained because of the limitation that the total number of background workers launched for purposes of parallel query cannot exceed [max_parallel_workers (integer)
       
        max_parallel_workers configuration parameter](braised:ref/runtime-config-resource#max-parallel-workers-integer-max-parallel-workers-configuration-parameter).
