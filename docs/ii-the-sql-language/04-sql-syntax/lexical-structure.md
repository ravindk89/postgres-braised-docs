...
The first few tokens are generally the command name, so in the above example we would usually speak of a "SELECT", an "UPDATE", and an "INSERT" command.
...
The precise syntax rules for each command are described in [Reference](#reference).

## Identifiers and Key Words

Tokens such as SELECT, UPDATE, or VALUES in the example above are examples of key words, that is, words that have a fixed meaning in the SQL language.
...
A complete list of key words can be found in [C. SQL Key Words](braised:ref/sql-keywords-appendix).
...
`NAMEDATALEN`-1 bytes of an identifier; longer names can be written in commands, but they will be truncated.
By default, `NAMEDATALEN` is 64 so the maximum identifier length is 63 bytes.
If this limit is problematic, it can be raised by changing the `NAMEDATALEN` constant in `src/include/pg_config_manual.h`.
...
delimited identifier or quoted identifier.
...
This variant starts with `U&` (upper or lower case U followed by ampersand) immediately before the opening double quote, without any spaces in between, for example `U&"foo"`. (Note that this creates an ambiguity with the operator `&`.
...
If a different escape character than backslash is desired, it can be specified using the `UESCAPE`
...
Note that the escape character is written in single quotes, not double quotes, after `UESCAPE`.
...
:::{.table}
  ...
  :::{/cell}
  :::{.cell}
  xadecimal byte value
  :::{/cell}
  :::{/row}
  :::{.row}
  :::{.cell}
  `\uxxxx`, `\Uxxxxxxxx` (*x* = 09, AF)   1
  :::{/cell}
  :::{.cell}
  or 32-bit hexadecimal Unicode character value
  :::{/cell}
  :::{/row}
:::{/table}

Any other character following a backslash is taken literally. 
Thus, to include a backslash character, write two backslashes (`\\`). Also, a single quote can be included in an escape string by writing `\'`, in addition to the normal way of `''`.
...
If the configuration parameter [standard_conforming_strings](braised:ref/runtime-config-compatible#standard-conforming-strings-boolean-stringsstandard-conforming-standard-conforming-strings-configuration-parameter) is `off`, then PostgreSQL recognizes backslash escapes in both regular and escape string constants. 
...
In addition to `standard_conforming_strings`, the configuration parameters [escape_string_warning](braised:ref/runtime-config-compatible#escape-string-warning-boolean-stringsescape-warning-escape-string-warning-configuration-parameter) and [backslash_quote ](braised:ref/runtime-config-compatible#backslash-quote-enum-stringsbackslash-quotes-backslash-quote-configuration-parameter) govern treatment of backslashes in string constants.
:::
...
A Unicode escape string constant starts with `U&` (upper or lower case letter U followed by ampersand) immediately before the opening quote, without any spaces in between, for example `U&'foo'`. (Note that this creates an ambiguity with the operator `&`.
Use spaces around the operator to avoid this problem.) Inside the quotes, Unicode characters can be specified in escaped form by writing a backslash followed by the four-digit hexadecimal code point number or alternatively a backslash followed by a plus sign followed by a six-digit hexadecimal code point number.
...
Also, the Unicode escape syntax for string constants only works when the configuration parameter [standard_conforming_strings (boolean)
      stringsstandard conforming
      
       standard_conforming_strings configuration parameter](braised:ref/runtime-config-compatible#standard-conforming-strings-boolean-stringsstandard-conforming-standard-conforming-strings-configuration-parameter) is turned on. This is because otherwise this syntax could confuse clients that parse the SQL statements to the point that it could lead to SQL injections and similar security issues. If the parameter is set to off, this syntax will be rejected with an error message.

### Dollar-Quoted String Constants

While the standard syntax for specifying string constants is usually convenient, it can be difficult to understand when the desired string contains many single quotes, since each of those must be doubled.
...
Tags are case sensitive, so `$tag$String content$tag$` is correct, but `$TAG$String content$tag$` is not.
...
Numeric constants are accepted in these general forms: *digits* *digits*.\[*digits*\]\[e\[+-\]*digits*\] \[*digits*\].*digits*\[e\[+-\]*digits*\] *digits*e\[+-\]*digits* where *digits* is one or more decimal digits (0 through 9).
...
There cannot be any spaces or other characters embedded in the constant, except for underscores, which can be used for visual grouping as described below.
...
Additionally, non-decimal integer constants are accepted in these forms: 0x*hexdigits* 0o*octdigits* 0b*bindigits* where *hexdigits* is one or more hexadecimal digits (0-9, A-F), *octdigits* is one or more octal digits (0-7), and *bindigits* is one or more binary digits (0 or 1).
Hexadecimal digits and the radix prefixes can be in upper or lower case.
Note that only integers can have non-decimal forms, not numbers with fractional parts.
...
These have no further effect on the value of the constant.
For example:
...
`integer` if its value fits in type `integer` (32 bits); otherwise it is presumed to be type `bigint` if its value fits in type `bigint` (64 bits); otherwise it is taken to be type `numeric`.
...
In most cases the constant will be automatically coerced to the most appropriate type depending on context.
When necessary, you can force a numeric value to be interpreted as a specific data type by casting it.`real` (`float4`) by writing:
...
A constant of an *arbitrary* type can be entered using any one of the following notations: *type* \'*string*\' \'*string*\'::*type* CAST ( \'*string*\' AS *type* ) The string constant\'s text is passed to the input conversion routine for the type called *type*.
...
It is also possible to specify a type coercion using a function-like syntax: *typename* ( \'*string*\' ) but not all type names can be used in this way; see [Type Casts](braised:ref/sql-expressions#type-casts) for details.

The `::`, `CAST()`, and function-call syntaxes can also be used to specify run-time type conversions of arbitrary expressions, as discussed in [Type Casts](braised:ref/sql-expressions#type-casts).
...
For example, if you have defined a prefix operator named `@`, you cannot write `X*@Y`; you must write `X* @Y` to ensure that PostgreSQL reads it as two operator names not one.
...
-   Brackets (`[]`) are used to select the elements of an array. See [Arrays](braised:ref/arrays) for more information on arrays.

-   Commas (`,`) are used in some syntactical constructs to separate the elements of a list.

-   The semicolon (`;`) terminates an SQL command. It cannot appear anywhere within a command, except within a string constant or quoted identifier.

-   The colon (`:`) is used to select "slices" from arrays. (See [Arrays](braised:ref/arrays).) In certain SQL dialects (such as Embedded SQL), the colon is used to prefix variable names.
...
Operator Precedence (highest to lowest) shows the precedence and associativity of the operators in PostgreSQL.
Most operators have the same precedence and are left-associative.
The precedence and associativity of the operators is hard-wired into the parser.
Add parentheses if you want an expression with multiple operators to be parsed in some other way than what the precedence rules imply.
...
the `OPERATOR` construct is taken to have the default precedence shown in Operator Precedence (highest to lowest) for "any other operator". This is true no matter which specific operator appears inside `OPERATOR()`.
