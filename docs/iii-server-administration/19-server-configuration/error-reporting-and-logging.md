---
title: "19.8. Error Reporting and Logging"
...
:::{.dl}
:::{.item term="`log_destination` (`string`)"}
...
If `csvlog` is included in `log_destination`, log entries are output in "comma-separated value" (CSV) format, which is convenient for loading logs into programs. See Using CSV-Format Log Output for details. logging_collector must be enabled to generate CSV-format log output.

If `jsonlog` is included in `log_destination`, log entries are output in JSON format, which is convenient for loading logs into programs. See Using JSON-Format Log Output for details. logging_collector must be enabled to generate JSON-format log output.
...
On Windows, when you use the `eventlog` option for `log_destination`, you should register an event source and its library with the operating system so that the Windows Event Viewer can display event log messages cleanly. See [Registering Event Log on Windows](braised:ref/event-log-registration) for details.
:::
:::{/item}
:::{.item term="`logging_collector` (`boolean`)"}
...
:::
:::{/item}
:::{.item term="`log_directory` (`string`)"}
...
:::{/item}
:::{.item term="`log_file_mode` (`integer`)"}
...
This parameter can only be set in the `postgresql.conf` file or on the server command line.
:::{/item}
:::{.item term="`log_rotation_age` (`integer`)"}
...
:::{/item}
:::{.item term="`log_rotation_size` (`integer`)"}
...
:::{/item}
:::{.item term="`log_truncate_on_rotation` (`boolean`)"}
...
:::{/item}
:::{.item term="`syslog_facility` (`enum`)"}
...
:::{/item}
:::{.item term="`syslog_sequence_numbers` (`boolean`)"}
...
This parameter can only be set in the `postgresql.conf` file or on the server command line.
:::{/item}
:::{.item term="`syslog_split_messages` (`boolean`)"}
...
:::{.dl}
:::{.item term="`log_min_messages` (`enum`)"}
Controls which message levels are written to the server log. Valid values are `DEBUG5`, `DEBUG4`, `DEBUG3`, `DEBUG2`, `DEBUG1`, `INFO`, `NOTICE`, `WARNING`, `ERROR`, `LOG`, `FATAL`, and `PANIC`. Each level includes all the levels that follow it. The later the level, the fewer messages are sent to the log. The default is `WARNING`. Note that `LOG` has a different rank here than in [client_min_messages (enum)
      
   client_min_messages configuration parameter](braised:ref/runtime-config-client#client-min-messages-enum-client-min-messages-configuration-parameter). Only superusers and users with the appropriate `SET` privilege can change this setting.
:::{/item}
:::{.item term="`log_min_error_statement` (`enum`)"}
...
:::{/item}
:::{.item term="`log_min_duration_statement` (`integer`)"}
...
:::
:::{/item}
:::{.item term="`log_min_duration_sample` (`integer`)"}
...
:::{/item}
:::{.item term="`log_statement_sample_rate` (`floating point`)"}
...
:::{/item}
:::{.item term="`log_transaction_sample_rate` (`floating point`)"}
...
:::
:::{/item}
:::{.item term="`log_startup_progress_interval` (`integer`)"}
...
Message Severity Levels explains the message severity levels used by PostgreSQL.
If logging output is sent to `syslog` or Windows\' `eventlog`, the severity levels are translated as shown in the table.
...
:::{.callout type="note"}
What you choose to log can have security implications; see [Log File Maintenance](braised:ref/logfile-maintenance).
:::

:::{.dl}
:::{.item term="`application_name` (`string`)"}
...
:::{/item}
:::{.item term="`debug_print_parse` (`boolean`); `debug_print_rewritten` (`boolean`); `debug_print_plan` (`boolean`)"}
These parameters enable various debugging output to be emitted. When set, they print the resulting parse tree, the query rewriter output, or the execution plan for each executed query. These messages are emitted at `LOG` message level, so by default they will appear in the server log but will not be sent to the client. You can change that by adjusting [client_min_messages (enum)
      
   client_min_messages configuration parameter](braised:ref/runtime-config-client#client-min-messages-enum-client-min-messages-configuration-parameter) and/or log_min_messages. These parameters are off by default.
:::{/item}
:::{.item term="`debug_pretty_print` (`boolean`)"}
...
:::{/item}
:::{.item term="`log_autovacuum_min_duration` (`integer`)"}
...
:::{/item}
:::{.item term="`log_checkpoints` (`boolean`)"}
...
:::{/item}
:::{.item term="`log_connections` (`string`)"}
...
:::
:::{/item}
:::{.item term="`log_disconnections` (`boolean`)"}
...
:::{/item}
:::{.item term="`log_duration` (`boolean`)"}
...
:::
:::{/item}
:::{.item term="`log_error_verbosity` (`enum`)"}
Controls the amount of detail written in the server log for each message that is logged. Valid values are `TERSE`, `DEFAULT`, and `VERBOSE`, each adding more fields to displayed messages. `TERSE` excludes the logging of `DETAIL`, `HINT`, `QUERY`, and `CONTEXT` error information. `VERBOSE` output includes the `SQLSTATE` error code (see also [A. PostgreSQL Error Codes](braised:ref/errcodes-appendix)) and the source code file name, function name, and line number that generated the error. Only superusers and users with the appropriate `SET` privilege can change this setting.
:::{/item}
:::{.item term="`log_hostname` (`boolean`)"}
...
:::{/item}
:::{.item term="`log_line_prefix` (`string`)"}
...
This parameter can only be set in the `postgresql.conf` file or on the server command line. The default is `'%m [%p] '` which logs a time stamp and the process ID.
  ...
  `%v`     Virtual transaction ID (procNumber/localXID); see [Transactions and Identifiers](braised:ref/transaction-id)                                                                                                                                                                           no
  `%x`     Transaction ID (0 if none is assigned); see [Transactions and Identifiers](braised:ref/transaction-id)                                                                                                                                                                                 no
  `%q`     Produces no output, but tells non-session processes to stop at this point in the string; ignored by session processes                                                                                                                              no
  `%Q`     Query identifier of the current query. Query identifiers are not computed by default, so this field will be zero unless [compute_query_id (enum)
      
   compute_query_id configuration parameter](braised:ref/runtime-config-statistics#compute-query-id-enum-compute-query-id-configuration-parameter) parameter is enabled or a third-party module that computes query identifiers is configured.   yes
  `%%`     Literal `%`                                                                                                                                                                                                                                        no
...
:::
:::{/item}
:::{.item term="`log_lock_waits` (`boolean`)"}
Controls whether a log message is produced when a session waits longer than [deadlock_timeout (integer)
   ...
   deadlock_timeout configuration parameter](braised:ref/runtime-config-locks#deadlock-timeout-integer-deadlock-timeout-during-timeout-deadlock-deadlock-timeout-configuration-parameter) to acquire a lock. This is useful in determining if lock waits are causing poor performance. The default is `off`. Only superusers and users with the appropriate `SET` privilege can change this setting.
:::{/item}
:::{.item term="`log_lock_failures` (`boolean`)"}
...
:::{/item}
:::{.item term="`log_recovery_conflict_waits` (`boolean`)"}
...
:::{/item}
:::{.item term="`log_parameter_max_length` (`integer`)"}
...
:::{/item}
:::{.item term="`log_parameter_max_length_on_error` (`integer`)"}
...
:::
:::{/item}
:::{.item term="`log_replication_commands` (`boolean`)"}
Causes each replication command and `walsender` process\'s replication slot acquisition/release to be logged in the server log. See [Streaming Replication Protocol](braised:ref/protocol-replication) for more information about replication command. The default value is `off`. Only superusers and users with the appropriate `SET` privilege can change this setting.
:::{/item}
:::{.item term="`log_temp_files` (`integer`)"}
...
:::{/item}
:::{.item term="`log_timezone` (`string`)"}
Sets the time zone used for timestamps written in the server log. Unlike [TimeZone (string)
      
   TimeZone configuration parameter
      
  time zone](braised:ref/runtime-config-client#timezone-string-timezone-configuration-parameter-time-zone), this value is cluster-wide, so that all sessions will report timestamps consistently. The built-in default is `GMT`, but that is typically overridden in `postgresql.conf`; initdb will install a setting there corresponding to its system environment. See [Time Zones](braised:ref/datatype-datetime#time-zones) for more information. This parameter can only be set in the `postgresql.conf` file or on the server command line.
:::{/item}
:::{/dl}
...
It is also possible to access the file as a foreign table, using the supplied [F.15. file_fdw — access data files in the server's file system](braised:ref/file-fdw) module.
...
Including `jsonlog` in the `log_destination` list provides a convenient way to import log files into many different programs.
This option emits log lines in JSON format.
...
Each log line is serialized as a JSON object with the set of keys and their associated values shown in Keys and Values of JSON Log Entries.
...
These settings control how process titles of server processes are modified. Process titles are typically viewed using programs like ps or, on Windows, Process Explorer. See [Standard Unix Tools](braised:ref/monitoring-ps) for details.

:::{.dl}
:::{.item term="`cluster_name` (`string`)"}
Sets a name that identifies this database cluster (instance) for various purposes. The cluster name appears in the process title for all server processes in this cluster. Moreover, it is the default application name for a standby connection (see [synchronous_standby_names (string)
      
   synchronous_standby_names configuration parameter](braised:ref/runtime-config-replication#synchronous-standby-names-string-synchronous-standby-names-configuration-parameter)).
...
:::{/item}
:::{.item term="`update_process_title` (`boolean`)"}
