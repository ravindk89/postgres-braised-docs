...
However, some of them can be used to assist with the recovery of severely damaged databases.
...
:::{.dl}
:::{.item term="`allow_in_place_tablespaces` (`boolean`)"}
...
:::{/item}
:::{.item term="`allow_system_table_mods` (`boolean`)"}
...
:::{/item}
:::{.item term="`backtrace_functions` (`string`)"}
...
:::{/item}
:::{.item term="`debug_copy_parse_plan_trees` (`boolean`)"}
...
:::{/item}
:::{.item term="`debug_discard_caches` (`integer`)"}
...
:::{/item}
:::{.item term="`debug_io_direct` (`string`)"}
...
:::{/item}
:::{.item term="`debug_parallel_query` (`enum`)"}
...
:::{/item}
:::{.item term="`debug_raw_expression_coverage_test` (`boolean`)"}
...
This parameter is only available when `DEBUG_NODE_TESTS_ENABLED` was defined at compile time (which happens automatically when using the configure option `--enable-cassert`).
:::{/item}
:::{.item term="`debug_write_read_parse_plan_trees` (`boolean`)"}
...
This parameter is only available when `DEBUG_NODE_TESTS_ENABLED` was defined at compile time (which happens automatically when using the configure option `--enable-cassert`).
:::{/item}
:::{.item term="`ignore_system_indexes` (`boolean`)"}
...
:::{/item}
:::{.item term="`post_auth_delay` (`integer`)"}
...
:::{/item}
:::{.item term="`pre_auth_delay` (`integer`)"}
...
:::{/item}
:::{.item term="`trace_notify` (`boolean`)"}
Generates a great amount of debugging output for the `LISTEN` and `NOTIFY` commands. [client_min_messages (enum)
      
   client_min_messages configuration parameter](braised:ref/runtime-config-client#client-min-messages-enum-client-min-messages-configuration-parameter) or [log_min_messages (enum)
      
   log_min_messages configuration parameter](braised:ref/runtime-config-logging#log-min-messages-enum-log-min-messages-configuration-parameter) must be `DEBUG1` or lower to send this output to the client or server logs, respectively.
:::{/item}
:::{.item term="`trace_sort` (`boolean`)"}
If on, emit information about resource usage during sort operations.
:::{/item}
:::{.item term="`trace_locks` (`boolean`)"}
...
:::{/item}
:::{.item term="`trace_lwlocks` (`boolean`)"}
...
:::{/item}
:::{.item term="`wal_consistency_checking` (`string`)"}
...
:::{/item}
:::{.item term="`ignore_checksum_failure` (`boolean`)"}
Only has effect if [-k](braised:ref/app-initdb#k) are enabled.
...
:::{/item}
:::{.item term="`zero_damaged_pages` (`boolean`)"}
...
:::{/item}
:::{.item term="`ignore_invalid_pages` (`boolean`)"}
...
:::{/item}
:::{.item term="`jit_debugging_support` (`boolean`)"}
...
:::{/item}
:::{.item term="`jit_dump_bitcode` (`boolean`)"}
Writes the generated LLVM IR out to the file system, inside [data_directory (string)
      
   data_directory configuration parameter](braised:ref/runtime-config-file-locations#data-directory-string-data-directory-configuration-parameter). This is only useful for working on the internals of the JIT implementation. The default setting is `off`. Only superusers and users with the appropriate `SET` privilege can change this setting.
:::{/item}
:::{.item term="`jit_expressions` (`boolean`)"}
Determines whether expressions are JIT compiled, when JIT compilation is activated (see [When to JIT?](braised:ref/jit-decision)). The default is `on`.
:::{/item}
:::{.item term="`jit_profiling_support` (`boolean`)"}
...
:::{/item}
:::{.item term="`jit_tuple_deforming` (`boolean`)"}
Determines whether tuple deforming is JIT compiled, when JIT compilation is activated (see [When to JIT?](braised:ref/jit-decision)). The default is `on`.
:::{/item}
:::{.item term="`remove_temp_files_after_crash` (`boolean`)"}
...
:::{/item}
:::{.item term="`send_abort_for_crash` (`boolean`)"}
...
:::{/item}
:::{.item term="`send_abort_for_kill` (`boolean`)"}
...
:::{/item}
:::{.item term="`debug_logical_replication_streaming` (`enum`)"}
