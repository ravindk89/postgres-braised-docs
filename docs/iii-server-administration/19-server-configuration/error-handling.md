...
:::{.dl}
:::{.item term="`exit_on_error` (`boolean`)"}
...
:::{/item}
:::{.item term="`restart_after_crash` (`boolean`)"}
...
:::{/item}
:::{.item term="`data_sync_retry` (`boolean`)"}
...
:::{/item}
:::{.item term="`recovery_init_sync_method` (`enum`)"}
