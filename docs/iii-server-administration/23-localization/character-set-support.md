---
title: "23.3. Character Set Support"
id: multibyte
---
...
Notice that the above commands specify copying the `template0` database. When copying any other database, the encoding and locale settings cannot be changed from those of the source database, because that might result in corrupt data. For more information see [Template Databases](braised:ref/manage-ag-templatedbs).
...
To enable automatic character set conversion, you have to tell PostgreSQL the character set (encoding) you would like to use in the client.
...
-   libpq ([Control Functions](braised:ref/libpq-control)) has functions to control the client encoding.
...
PostgreSQL allows conversion between any two character sets for which a conversion function is listed in the [pg_conversion](#catalog-pg-conversion) system catalog.
PostgreSQL comes with some predefined conversions, as summarized in Built-in Client/Server Character Set Conversions and shown in more detail in All Built-in Character Set Conversions.
You can create a new conversion using the SQL command [CREATE CONVERSION](braised:ref/sql-createconversion). (To be used for automatic client/server conversions, a conversion must be marked as "default" for its character set pair.)
