---
title: "18.6. Upgrading a PostgreSQL Cluster"
id: upgrading
---
...
Current PostgreSQL version numbers consist of a major and a minor version number.
...
For releases before PostgreSQL version 10.0, version numbers consist of three numbers, for example, 9.5.3.
In those cases, the major version consists of the first two digit groups of the version number, e.g., 9.5, and the minor version is the third number, e.g., 3, meaning this would be the third minor release of the major release 9.5.
...
For example, version 10.1 is compatible with version 10.0 and version 10.6.
Similarly, for example, 9.5.3 is compatible with 9.5.0, 9.5.1, and 9.5.6.
...
A faster method is [pg_upgrade](braised:ref/pgupgrade).
Replication methods are also available, as discussed below. (If you are using a pre-packaged version of PostgreSQL, it may provide scripts to assist with major version upgrades.
...
All user-visible changes are listed in the release notes ([Release Notes](#release-notes)); pay particular attention to the section labeled \"Migration\".
...
-   Shut down the old server:
    ...
    See [Server Setup and Operation](#server-setup-and-operation) for details about starting and stopping the server.
...
-   Install the new version of PostgreSQL as outlined in [Installation from Source Code](#installation-from-source-code).
...
The [pg_upgrade](braised:ref/pgupgrade) module allows an installation to be migrated in-place from one major PostgreSQL version to another.
...
The pg_upgrade [documentation](#pgupgrade) outlines the necessary steps.
...
Once it has synced up with the primary server (running the older version of PostgreSQL), you can switch primaries and make the standby the primary and shut down the older database instance.
