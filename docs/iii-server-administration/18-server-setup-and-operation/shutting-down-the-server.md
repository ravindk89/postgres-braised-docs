---
title: "18.5. Shutting Down the Server"
id: server-shutdown
---
...
Under the hood, they all reduce to sending a signal to the supervisor `postgres` process.

If you are using a pre-packaged version of PostgreSQL, and you used its provisions for starting the server, then you should also use its provisions for stopping the server.
...
The [pg_ctl](braised:ref/app-pg-ctl) program provides a convenient interface for sending these signals to shut down the server.
Alternatively, you can send the signal directly using `kill` on non-Windows systems.
The PID of the `postgres` process can be found using the `ps` program, or from the file `postmaster.pid` in the data directory.
...
To terminate an individual session while allowing other sessions to continue, use `pg_terminate_backend()` (see [Server Signaling Functions](braised:ref/functions-admin#server-signaling-functions)) or send a `SIGTERM` signal to the child process associated with the session.
