---
title: "28.3. Write-Ahead Logging (WAL)"
id: wal-intro
---
...
WAL also makes it possible to support on-line backup and point-in-time recovery, as described in [Continuous Archiving and Point-in-Time Recovery (PITR)](braised:ref/continuous-archiving).
By archiving the WAL data we can support reverting to any time instant covered by the available WAL data: we simply install a prior physical backup of the database, and replay the WAL just as far as the desired time.
What\'s more, the physical backup doesn\'t have to be an instantaneous snapshot of the database state if it is made over some period of time, then replaying the WAL for that period will fix any internal inconsistencies.
