...
However, clients can open multiple connections to the same database, or different databases.
Database-level security has two components: access control (see [The pg_hba.conf File](braised:ref/auth-pg-hba-conf)), managed at the connection level, and authorization control (see [Privileges](braised:ref/ddl-priv)), managed via the grant system.
Foreign data wrappers (see [F.38. postgres_fdw — access data stored in external PostgreSQL servers](braised:ref/postgres-fdw)) allow for objects within one database to act as proxies for objects in other database or clusters.
The older dblink module (see [F.11. dblink — connect to other PostgreSQL databases](braised:ref/dblink)) provides a similar capability.
...
More information about managing schemas is in [Schemas](braised:ref/ddl-schemas).
...
In particular, the impact that having a shared WAL (see [Reliability and the Write-Ahead Log](#reliability-and-the-write-ahead-log)) has on backup and recovery options.
...
Databases are created with the `CREATE DATABASE` command (see [Creating a Database](braised:ref/manage-ag-createdb)) and destroyed with the `DROP DATABASE` command (see [Destroying a Database](braised:ref/manage-ag-dropdb)).
To determine the set of existing databases, examine the pg_database system catalog, for example SELECT datname FROM pg_database; The [psql](braised:ref/app-psql) program\'s `\l` meta-command and `-l` command-line option are also useful for listing the existing databases.
