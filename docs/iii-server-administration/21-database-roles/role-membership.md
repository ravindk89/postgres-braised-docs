...
Once the group role exists, you can add and remove members using the [`GRANT`](#sql-grant) and [`REVOKE`](#sql-revoke) commands: GRANT *group_role* TO *role1*, \... ; REVOKE *group_role* FROM *role1*, \... ; You can grant membership to other group roles, too (since there isn\'t really any distinction between group roles and non-group roles).
...
First, member roles that have been granted membership with the `SET` option can do [`SET ROLE`](#sql-set-role) to temporarily "become" the group role.
...
the session would have use of only those privileges granted to `admin`, and not those granted to `joe` or `island`.
After:

    SET ROLE wheel;

the session would have use of only those privileges granted to `wheel`, and not those granted to either `joe` or `admin`.
...
Continuing the above example, we might choose to grant `CREATEDB` and `CREATEROLE` to the `admin` role.
Then a session connecting as role `joe` would not have these privileges immediately, only after doing `SET ROLE admin`.
