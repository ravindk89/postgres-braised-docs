---
title: "34.11. Library Functions"
id: ecpg-library
---

## Library Functions

The `libecpg` library primarily contains "hidden" functions that are used to implement the functionality expressed by the embedded SQL commands.
But there are some functions that can usefully be called directly.
Note that this makes your code unportable.
...
-   `ECPGtransactionStatus(const char *connection_name)` returns the current transaction status of the given connection identified by *connection_name*. See [Connection Status Functions](braised:ref/libpq-status) and libpq\'s `PQtransactionStatus` for details about the returned status codes.
