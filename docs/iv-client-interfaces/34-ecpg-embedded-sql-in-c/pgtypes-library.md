---
title: "34.6. pgtypes Library"
id: ecpg-pgtypes
---

## pgtypes Library

The pgtypes library maps PostgreSQL database types to C equivalents that can be used in C programs.
It also offers functions to do basic calculations with those types within C, i.e., without the help of the PostgreSQL server.
...
Some functions such as `PGTYPESnumeric_to_asc` return a pointer to a freshly allocated character string.
These results should be freed with `PGTYPESchar_free` instead of `free`. (This is important only on Windows, where memory allocation and release sometimes need to be done by the same library.)

### The numeric Type

The numeric type offers to do calculations with arbitrary precision.
See [Numeric Types](braised:ref/datatype-numeric) for the equivalent type in the PostgreSQL server.
Because of the arbitrary precision this variable needs to be able to expand and shrink dynamically.
That\'s why you can only create numeric variables on the heap, by means of the `PGTYPESnumeric_new` and `PGTYPESnumeric_free` functions.
The decimal type, which is similar but limited in precision, can be created on the stack as well as on the heap.

The following functions can be used to work with the numeric type:

:::{.dl}
:::{.item term="`PGTYPESnumeric_new`"}
Request a pointer to a newly allocated numeric variable. numeric \*PGTYPESnumeric_new(void);
:::{/item}
:::{.item term="`PGTYPESnumeric_free`"}
Free a numeric type, release all of its memory. void PGTYPESnumeric_free(numeric \*var);
:::{/item}
:::{.item term="`PGTYPESnumeric_from_asc`"}
...
:::{/item}
:::{.item term="`PGTYPESnumeric_to_asc`"}
...
:::{/item}
:::{.item term="`PGTYPESnumeric_from_int`"}
...
:::{/item}
:::{.item term="`PGTYPESnumeric_from_long`"}
...
:::{/item}
:::{.item term="`PGTYPESnumeric_from_double`"}
...
:::{/item}
:::{.item term="`PGTYPESnumeric_to_double`"}
...
:::{/item}
:::{.item term="`PGTYPESnumeric_to_int`"}
...
:::{/item}
:::{.item term="`PGTYPESnumeric_to_long`"}
...
:::{/item}
:::{.item term="`PGTYPESnumeric_to_decimal`"}
...
:::{/item}
:::{.item term="`PGTYPESnumeric_from_decimal`"}
...
The date type in C enables your programs to deal with data of the SQL type date.
See [Date/Time Types](braised:ref/datatype-datetime) for the equivalent type in the PostgreSQL server.

The following functions can be used to work with the date type:

:::{.dl}
:::{.item term="`PGTYPESdate_from_timestamp`"}
...
:::{/item}
:::{.item term="`PGTYPESdate_from_asc`"}
...
Valid Input Formats for  shows the allowed input formats.
...
:::{/item}
:::{.item term="`PGTYPESdate_dayofweek`"}
...
Valid Input Formats for  indicates a few possible formats. This will give you an idea of how to use this function. All output lines are based on the same date: November 23, 1959.
...
:::{/item}
:::{.item term="`PGTYPESdate_defmt_asc`"}
...
Valid Input Formats for  indicates a few possible formats. This will give you an idea of how to use this function.
...
The timestamp type in C enables your programs to deal with data of the SQL type timestamp.
See [Date/Time Types](braised:ref/datatype-datetime) for the equivalent type in the PostgreSQL server.

The following functions can be used to work with the timestamp type:

:::{.dl}
:::{.item term="`PGTYPEStimestamp_from_asc`"}
...
Valid Input Formats for  contains a few examples for input strings.
...
:::{/item}
:::{.item term="`PGTYPEStimestamp_to_asc`"}
...
:::{/item}
:::{.item term="`PGTYPEStimestamp_current`"}
...
:::{/item}
:::{.item term="`PGTYPEStimestamp_fmt_asc`"}
...
-   `%%` - is replaced by `%`.
:::{/item}
:::{.item term="`PGTYPEStimestamp_sub`"}
...
Upon success, the function returns 0 and a negative value if an error occurred.
:::{/item}
:::{.item term="`PGTYPEStimestamp_defmt_asc`"}
...
This is the reverse function to `pgtypestimestampfmtasc`. See the documentation there in order to find out about the possible formatting mask entries.
:::{/item}
:::{.item term="`PGTYPEStimestamp_add_interval`"}
...
Upon success, the function returns 0 and a negative value if an error occurred.
:::{/item}
:::{.item term="`PGTYPEStimestamp_sub_interval`"}
...
:::{/item}
:::{.item term="`PGTYPESinterval_to_asc`"}
...
:::{/item}
:::{.item term="`PGTYPESinterval_copy`"}
...
However it is limited to a maximum precision of 30 significant digits.
In contrast to the numeric type which can be created on the heap only, the decimal type can be created either on the stack or on the heap (by means of the functions `PGTYPESdecimal_new` and `PGTYPESdecimal_free`).
There are a lot of other functions that deal with the decimal type in the Informix compatibility mode described in [Informix Compatibility Mode](braised:ref/ecpg-informix-compat).

The following functions can be used to work with the decimal type and are not only contained in the `libcompat` library.

:::{.dl}
:::{.item term="`PGTYPESdecimal_new`"}
Request a pointer to a newly allocated decimal variable. decimal \*PGTYPESdecimal_new(void);
:::{/item}
:::{.item term="`PGTYPESdecimal_free`"}
...
:::{.dl}
:::{.item term="`PGTYPES_NUM_BAD_NUMERIC`"}
...
:::{/item}
:::{.item term="`PGTYPES_NUM_OVERFLOW`"}
...
:::{/item}
:::{.item term="`PGTYPES_NUM_UNDERFLOW`"}
...
:::{/item}
:::{.item term="`PGTYPES_TS_ERR_EINFTIME`"}
...
:::{.dl}
:::{.item term="`PGTYPESInvalidTimestamp`"}
