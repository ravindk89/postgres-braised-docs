---
title: "34.15. Informix Compatibility Mode"
id: ecpg-informix-compat
---

## Informix Compatibility Mode

`ecpg` can be run in a so-called Informix compatibility mode.
If this mode is active, it tries to behave as if it were the Informix precompiler for Informix E/SQL.
Generally spoken this will allow you to use the dollar sign instead of the `EXEC SQL` primitive to introduce embedded SQL commands:
...
Informix compatibility mode is closely connected to the pgtypeslib library of ECPG. pgtypeslib maps SQL data types to data types within the C host program and most of the additional functions of the Informix compatibility mode allow you to operate on those C host program types.
Note however that the extent of the compatibility is limited.
It does not try to copy Informix behavior; it allows you to do more or less the same operations and gives you functions that have the same name and the same basic behavior but it is no drop-in replacement if you are using Informix at the moment.
Moreover, some of the data types are different.
For example, PostgreSQL\'s datetime and interval types do not know about ranges like for example `YEAR TO MINUTE` so you won\'t find support in ECPG for that either.

### Additional Types

The Informix-special \"string\" pseudo-type for storing right-trimmed character string data is now supported in Informix-mode without using `typedef`.
In fact, in Informix-mode, ECPG refuses to process source files that contain `typedef sometype string;`
...
Informix-compatible mode supports a different structure than the one described in [SQLDA Descriptor Areas](braised:ref/ecpg-descriptors#sqlda-descriptor-areas).
See below:
...
:::{/item}
:::{.item term="`sqltypename`; `sqltypelen`; `sqlownerlen`; `sqlsourcetype`; `sqlownername`; `sqlsourceid`; `sqlflags`; `sqlreserved`"}
Unused.
:::{/item}
:::{.item term="`sqlilongdata`"}
...
Internally this function uses the [PGTYPESdate_today](braised:ref/ecpg-pgtypes#pgtypesdate-today) function.
:::{/item}
:::{.item term="`rjulmdy`"}
...
Internally the function uses the [PGTYPESdate_julmdy](braised:ref/ecpg-pgtypes#pgtypesdate-julmdy) function.
:::{/item}
:::{.item term="`rdefmtdate`"}
...
Internally this function is implemented to use the [PGTYPESdate_defmt_asc](braised:ref/ecpg-pgtypes#pgtypesdate-defmt-asc) function. See the reference there for a table of example input.
:::{/item}
:::{.item term="`rfmtdate`"}
...
Internally this function uses the [PGTYPESdate_fmt_asc](braised:ref/ecpg-pgtypes#pgtypesdate-fmt-asc) function, see the reference there for examples.
:::{/item}
:::{.item term="`rmdyjul`"}
...
Internally the function is implemented to use the function [PGTYPESdate_mdyjul](braised:ref/ecpg-pgtypes#pgtypesdate-mdyjul).
:::{/item}
:::{.item term="`rdayofweek`"}
...
Internally the function is implemented to use the function [PGTYPESdate_dayofweek](braised:ref/ecpg-pgtypes#pgtypesdate-dayofweek).
:::{/item}
:::{.item term="`dtcurrent`"}
...
Internally this function uses the [PGTYPEStimestamp_from_asc](braised:ref/ecpg-pgtypes#pgtypestimestamp-from-asc) function. See the reference there for a table with example inputs.
:::{/item}
:::{.item term="`dtcvfmtasc`"}
...
This function is implemented by means of the [PGTYPEStimestamp_defmt_asc](braised:ref/ecpg-pgtypes#pgtypestimestamp-defmt-asc) function. See the documentation there for a list of format specifiers that can be used.
...
Internally, this function uses the [PGTYPEStimestamp_fmt_asc](braised:ref/ecpg-pgtypes#pgtypestimestamp-fmt-asc) function. See the reference there for information on what format mask specifiers can be used.
:::{/item}
:::{.item term="`intoasc`"}
...
:::{/item}
:::{.item term="`risnull`"}
Test if a variable is NULL. int risnull(int t, char \*ptr); The function receives the type of the variable to test (`t`) as well a pointer to this variable (`ptr`). Note that the latter needs to be cast to a char\*. See the function `rsetnull` for a list of possible variable types.
...
In the descriptions of the different constants you can also find the value that the constants represent in the current implementation.
However you should not rely on this number.
You can however rely on the fact all of them are defined to represent negative values.

:::{.dl}
:::{.item term="`ECPG_INFORMIX_NUM_OVERFLOW`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_NUM_UNDERFLOW`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_DIVIDE_ZERO`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_BAD_YEAR`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_BAD_MONTH`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_BAD_DAY`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_ENOSHORTDATE`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_DATE_CONVERT`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_OUT_OF_MEMORY`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_ENOTDMY`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_BAD_NUMERIC`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_BAD_EXPONENT`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_BAD_DATE`"}
...
:::{/item}
:::{.item term="`ECPG_INFORMIX_EXTRA_CHARS`"}
