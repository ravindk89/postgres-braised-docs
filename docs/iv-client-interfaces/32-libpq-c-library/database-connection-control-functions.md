---
title: "32.1. Database Connection Control Functions"
id: libpq-connect
---
...
An application program can have several backend connections open at one time. (One reason to do that is to access more than one database.) Each connection is represented by a PGconn`PQconnectdb`, `PQconnectdbParams`, or `PQsetdbLogin`.
Note that these functions will always return a non-null object pointer, unless perhaps there is too little memory even to allocate the PGconn object.
The `PQstatus` function should be called to check the return value for a successful connection before queries are sent via the connection object.
...
:::{/item}
:::{.item term="`PQconnectStartParams`; `PQconnectStart`; `PQconnectPoll`"}
...
:::{/item}
:::{.item term="`PQsocketPoll`"}
`PQsocket`. The primary use of this function is iterating through the connection sequence described in the documentation of `PQconnectStartParams`. typedef int64_t pg_usec_time_t; int PQsocketPoll(int sock, int forRead, int forWrite, pg_usec_time_t end_time);
...
:::{/item}
:::{.item term="`PQresetStart`; `PQresetPoll`"}
...
:::{/item}
:::{.item term="`PQsetSSLKeyPassHook_OpenSSL`"}
...
`PQsetSSLKeyPassHook_OpenSSL` lets an application override libpq's default handling of encrypted client certificate key files using or interactive prompting. void PQsetSSLKeyPassHook_OpenSSL(PQsslKeyPassHook_OpenSSL_type hook); The application passes a pointer to a callback function with signature:
...
:::{/item}
:::{.item term="`PQgetSSLKeyPassHook_OpenSSL`"}
...
Several libpq functions parse a user-specified string to obtain connection parameters.
There are two accepted formats for these strings: plain keyword/value strings and URIs.
URIs generally follow [RFC 3986](https://datatracker.ietf.org/doc/html/rfc3986), except that multi-host connection strings are allowed as further described below.
...
Spaces around a setting\'s equal sign are optional.
...
The connection URI needs to be encoded with [percent-encoding](https://datatracker.ietf.org/doc/html/rfc3986#section-2.1) if it includes symbols with special meaning in any of its parts.
Here is an example where the equal sign (`=`) is replaced with `%3D` and the space character with `%20`:

    postgresql://user@localhost:5433/mydb?options=-c%20synchronous_commit%3Doff

The host part may be either a host name or an IP address.
To specify an IPv6 address, enclose it in square brackets: postgresql://\[2001:db8::1234\]/database

The host part is interpreted as described for the parameter [host](#libpq-connect-host).
In particular, a Unix-domain socket connection is chosen if the host part is either empty or looks like an absolute path name, otherwise a TCP/IP connection is initiated.
Note, however, that the slash is a reserved character in the hierarchical part of the URI.
...
It is possible to specify multiple host components, each with an optional port component, in a single URI.
A URI of the form `postgresql://host1:port1,host2:port2,host3:port3/` is equivalent to a connection string of the form `host=host1,host2,host3 port=port1,port2,port3`.
...
The same number of elements must be given in each option that is specified, such that e.g., the first `hostaddr` corresponds to the first host name, the second `hostaddr` corresponds to the second host name, and so forth.
...
In either format, a single host name can translate to multiple network addresses.
A common example of this is a host that has both an IPv4 and an IPv6 address.
...
All the other connection options are the same for every host in the list; it is not possible to e.g., specify different usernames for different hosts.
...
:::{.dl}
:::{.item term="`host`"}
Name of host to connect to.`@`, it is taken as a Unix-domain socket in the abstract namespace (currently supported on Linux and Windows). The default behavior when `host` is not specified, or is empty, is to connect to a Unix-domain socket`/tmp` (or whatever socket directory was specified when PostgreSQL was built). On Windows, the default is to connect to `localhost`.
...
Note that authentication is likely to fail if `host` is not the name of the server at network address `hostaddr`. Also, when both `host` and `hostaddr` are specified, `host` is used to identify the connection in a password file (see [The Password File](braised:ref/libpq-pgpass)).
...
:::{/item}
:::{.item term="`passfile`"}
Specifies the name of the file used to store passwords (see [The Password File](braised:ref/libpq-pgpass)). Defaults to `~/.pgpass`, or `%APPDATA%\postgresql\pgpass.conf` on Microsoft Windows. (No error is reported if this file does not exist.)
:::{/item}
:::{.item term="`require_auth`"}
...
:::{/item}
:::{.item term="`options`"}
Specifies command-line options to send to the server at connection start. For example, setting this to `-c geqo=off` or `--geqo=off` sets the session\'s value of the `geqo` parameter to `off`. Spaces within this string are considered to separate command-line arguments, unless escaped with a backslash (`\`); write `\\` to represent a literal backslash. For a detailed discussion of the available options, consult [Server Configuration](#server-configuration).
:::{/item}
:::{.item term="`application_name`"}
Specifies a value for the [application_name (string)
      
   application_name configuration parameter](braised:ref/runtime-config-logging#application-name-string-application-name-configuration-parameter) configuration parameter.
:::{/item}
:::{.item term="`fallback_application_name`"}
Specifies a fallback value for the [application_name (string)
      
   application_name configuration parameter](braised:ref/runtime-config-logging#application-name-string-application-name-configuration-parameter) configuration parameter. This value will be used if no value has been given for `application_name` via a connection parameter or the `PGAPPNAME` environment variable. Specifying a fallback name is useful in generic utility programs that wish to set a default application name but allow it to be overridden by the user.
:::{/item}
:::{.item term="`keepalives`"}
...
See [SSL Support](braised:ref/libpq-ssl) for a detailed description of how these options work.
...
:::{/item}
:::{.item term="`ssl_max_protocol_version`"}
...
:::{/item}
:::{.item term="`min_protocol_version`"}
...
:::{/item}
:::{.item term="`max_protocol_version`"}
...
The current supported values are `3.0`, `3.2`, and `latest`. The `latest` value is equivalent to the latest protocol version supported by the libpq version being used, which is currently `3.2`.
:::{/item}
:::{.item term="`krbsrvname`"}
Kerberos service name to use when authenticating with GSSAPI. This must match the service name specified in the server configuration for Kerberos authentication to succeed. (See also [GSSAPI Authentication](braised:ref/gssapi-auth).) The default value is normally `postgres`, but that can be changed when building PostgreSQL via the `--with-krb-srvnam` option of configure. In most environments, this parameter never needs to be changed. Some Kerberos implementations might require a different service name, such as Microsoft Active Directory which requires the service name to be in upper case (`POSTGRES`).
:::{/item}
:::{.item term="`gsslib`"}
...
:::{/item}
:::{.item term="`scram_client_key`"}
The base64-encoded SCRAM client key. This can be used by foreign-data wrappers or similar middleware to enable pass-through SCRAM authentication. See [Connection Management Options](braised:ref/postgres-fdw#connection-management-options) for one such implementation. It is not meant to be specified directly by users or client applications.
:::{/item}
:::{.item term="`scram_server_key`"}
The base64-encoded SCRAM server key. This can be used by foreign-data wrappers or similar middleware to enable pass-through SCRAM authentication. See [Connection Management Options](braised:ref/postgres-fdw#connection-management-options) for one such implementation. It is not meant to be specified directly by users or client applications.
:::{/item}
:::{.item term="`service`"}
Service name to use for additional parameters. It specifies a service name in `pg_service.conf` that holds additional connection parameters. This allows applications to specify only a service name so connection parameters can be centrally maintained. See [The Connection Service File](braised:ref/libpq-pgservice).
:::{/item}
:::{.item term="`target_session_attrs`"}
...
:::{/item}
:::{.item term="`oauth_issuer`"}
The HTTPS URL of a trusted issuer to contact if the server requests an OAuth token for the connection. This parameter is required for all OAuth connections; it should exactly match the `issuer` setting in [the server\'s HBA configuration](#auth-oauth).
...
:::
:::{/item}
:::{.item term="`oauth_client_id`"}
An OAuth 2.0 client identifier, as issued by the authorization server. If the PostgreSQL server [requests an OAuth token](#auth-oauth) for the connection (and if no [custom OAuth hook](#libpq-oauth-authdata-hooks) is installed to provide one), then this parameter must be set; otherwise, the connection will fail.
:::{/item}
:::{.item term="`oauth_client_secret`"}
