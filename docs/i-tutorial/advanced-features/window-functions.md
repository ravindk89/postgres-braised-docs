...
When `ORDER BY` is omitted the default frame consists of all rows in the partition. [^1] Here is an example using `sum`:
...
More details about window functions can be found in [Window Function Calls](braised:ref/sql-expressions#window-function-calls), [Window Functions](braised:ref/functions-window), [Window Function Processing](braised:ref/queries-table-expressions#window-function-processing), and the [SELECT](braised:ref/sql-select) reference page.

[^1]: There are options to define the window frame in other ways, but this tutorial does not cover them.
See [Window Function Calls](braised:ref/sql-expressions#window-function-calls) for details.
