...
For example, to retrieve all the rows of table weather, type:
...
In this example, the sort order isn\'t fully specified, and so you might get the San Francisco rows in either order.
...
You can ensure consistent results by using `DISTINCT` and `ORDER BY` together: [^2]
...
[^2]: In some database systems, including older versions of PostgreSQL, the implementation of `DISTINCT` automatically orders the rows and so `ORDER BY` is unnecessary.
But this is not required by the SQL standard, and current PostgreSQL does not guarantee that `DISTINCT` causes the rows to be ordered.
