...
The `date` type is actually quite flexible in what it accepts, but for this tutorial we will stick to the unambiguous format shown here.
...
`COPY` to load large amounts of data from flat-text files.
...
The data inserted above into the weather table could also be inserted from a file containing (values are separated by a tab character):

    San Francisco    46    50    0.25    1994-11-27
    San Francisco    43    57    0.0    1994-11-29
    Hayward    37    54    \N    1994-11-29

You can read more about the `COPY` command in [COPY](braised:ref/sql-copy).
